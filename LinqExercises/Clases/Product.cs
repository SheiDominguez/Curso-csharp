...
namespace LinqExercises.Clases
{
    ...
    public static class Products
    {
        ...
        public static string GetFirstSeafood()
        {
            Product firstSeafood = Products.ProductList
                .Where(product => product.Category.Equals("Seafood"))
                .FirstOrDefault();

            return firstSeafood.ToString();
        }

        // Precio promedio de productos por categoria
        public static string AveragePricePerCategory()
        {
            var categories = ProductList
                .GroupBy(product => product.Category)
                .Select(category => new { Category = category.Key, AveragePrice = category.Average(prod => prod.UnitPrice) });

            StringBuilder sb = new StringBuilder();
            sb.Append("Precio promedio por categoría").Append("\n");

            foreach (var category in categories)
                sb.Append($"Categoría: {category.Category}").Append("\t\t").Append($"Precio: {category.AveragePrice}").Append("\n");

            return sb.ToString();
        }

        // cantidad de productos
        public static string ProductsAmount()
        {
            return $"Hay {ProductList.Count} productos.";
        }

        // productos que cuestan menos de 12 ordenados por el mas barato
        public static string ProductsPriceLower12AverageOrdered()
        {
            var productos = ProductList
                .Where(product => product.UnitPrice < 12.0000M)
        ...
        public static string Top3ExpensiveProducts()
        {
        ...
        public static string Top3CheaperProducts()
        {
            var products = ProductList
                .OrderBy(prod => prod.UnitPrice)
                .Take(3);
        ...
        public static string ProductWithLargestName()
        {
            var product = ProductList
                .OrderByDescending(prod => prod.ProductName.Length)
                .First();
        ...
        public static string GeneralStock()
        {
            var unidadesEnStock = ProductList
                .Select(product => product.UnitsInStock)
                .Sum();

            return $"Hay {unidadesEnStock} productos.";
        }

        // traer el nombre del producto con el id 80
        public static string ProductWithId80()
        {
            var product = ProductList
                .Where(prod => prod.ProductID == 80)
                .FirstOrDefault();

            return product != null ? product.ToString() : "No se encontró el producto";
        }

        // listado ordenado por la categoria y luego por el precio
        public static string ListOrderedByCategoryThenPrice()
        {
            var listado = ProductList.OrderBy(prod => prod.Category).ThenBy(prod => prod.UnitPrice);

            StringBuilder sb = new StringBuilder();

            foreach (var product in listado)
                sb.Append(product.ToString()).Append("\n");
        ...
        public static string ProductWithMoreWords()
        {
            var product = ProductList
                .OrderByDescending(prod => prod.ProductName.Split(' ').Length)
                .First();
        ...
        public static string ProductWithHigherTotalPrice()
        {
            var product = ProductList
                .OrderByDescending(prod => prod.UnitPrice * prod.UnitsInStock)
                .First();
